...
layui.define(["jquery","laytpl","kconfig"], function (exports) {
    var $ = layui.jquery;
    var laytpl = layui.laytpl;
    var kconfig = layui.kconfig;
    var hint = layui.hint();
            ...
            '{{# for(var i=0;i<d.length;i++){ }}',
                '<div class="eleTree-node" data-'+options.request.key+'="{{d[i]["'+options.request.key+'"]}}" eletree-floor="'+floor+'" style="display: none;">',
                    '<div class="eleTree-node-content" style="padding-left: '+(options.indent*floor)+'px;">',
                        ...
                        '<span class="eleTree-node-content-tools"><i class="layui-icon layui-icon-set"></i><span  class="content"></span></span>',
                    '</div>',
    ...
    Class.prototype={
        ...
        rightClickMenu: function() {
            ...
            var menuStr=['<ul id="tree-menu">'
                ...
                ,$.inArray("more",options.contextmenuList)!==-1?'<li class="more"><a href="javascript:;">更多</a></li>':''
            ,'</ul>'].join("");
            ...
            options.elem.off("contextmenu").on("contextmenu",".eleTree-node-content",function(e) {
                ...
                // 删除
                $("#tree-menu li.more").off().on("click",function(e) {
                    layer.msg('test');
                })
    ...
    layui.link(kconfig.resourcePath + 'css/eleTree.css');
