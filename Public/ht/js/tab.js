 ...
 * LICENSE:MIT
 */
layui.define(['jquery', 'element', 'nprogress', 'utils','md5'], function(exports) {
    var $ = layui.jquery,
        _modName = 'tab',
    ...
    var _tab = {
        ...
        tabDomExists: function() {
            var that = this;
            if (_doc.find('div.k-tab').length > 0) {
                that._title = $('.k-tab ul.layui-tab-title');
                that._content = $('.k-tab div.layui-tab-content');
                return true;
            }
            return false;
        },
        ...
        createTabDom: function() {
            ...
            //模板
            var tpl = [
                    '<div class="layui-tab layui-tab-card k-tab" lay-filter="' + that._filter + '">',
                    '<ul class="layui-tab-title">',
                    '<li class="layui-this" lay-id="-1" data-url="' + _config.mainUrl + '"><i class="layui-icon">&#xe68e;</i> 首页</li>',
                    '</ul>',
                    '<div class="k-tab-tool">操作&nbsp;<i class="fa fa-caret-down"></i></div>',
                    '<div class="k-tab-tool-body layui-anim layui-anim-upbit">',
                    '<ul>',
                    '<li class="k-item" data-target="refresh">刷新当前选项卡</li>',
                    '<li class="k-line"></li>',
                    '<li class="k-item" data-target="closeCurrent">关闭当前选项卡</li>',
                    '<li class="k-item" data-target="closeOther">关闭其他选项卡</li>',
                    '<li class="k-line"></li>',
                    '<li class="k-item" data-target="closeAll">关闭所有选项卡</li>',
                    '</ul>',
                    '</div>',
            ...
            switch (_config.renderType) {
                ...
                case renderType.iframe:
                    _htm = _htm.replace('{{content}}', '<iframe frameborder="0" marginheight="0" marginwidth="0" style="vertical-align:bottom" src="' + _config.mainUrl + '"></iframe>');
                    break;
            }
            //渲染
            ...
            that._title = $('.k-tab ul.layui-tab-title');
            that._content = $('.k-tab div.layui-tab-content');
            var _tool = $('.k-tab-tool'),
                _toolBody = $('.k-tab-tool-body');
            //监听操作点击事件
            _tool.on('click', function() {
                _toolBody.toggle();
            });
            //监听操作项点击事件
            _toolBody.find('li.k-item').each(function() {
                var $that = $(this);
        ...
        winResize: function() {
            ...
            _win.on('resize', function() {
                ...
                switch (_config.renderType) {
                    case renderType.page:
                        $('.k-tab .layui-tab-content').height(currBoxHeight - 40);
                        break;
                    case renderType.iframe:
                        $('.k-tab .layui-tab-content iframe').height(currBoxHeight - 40);
                        break;
                }
            }).resize();
        },
        ...
        tabAdd: function(options) {
            ...
            switch (_config.renderType) {
                case renderType.page:
                    ...
                    break;
                case renderType.iframe:
                    contentHtm = contentHtm.replace('{{content}}', '<iframe frameborder="0" marginheight="0" marginwidth="0" style="vertical-align:bottom"  src="' + url + '"></iframe>');
                    break;
            }
            //追加htm
